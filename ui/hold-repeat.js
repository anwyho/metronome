...
/* Steps per second when repeating begins, and at the ceiling. Starting slow is
   the point: the first second has to be usable for a nudge of two or three. */
...
export function stepsAfter(held) {
  const t = held - HOLD_DELAY;
  if (!(t > 0)) return 0;
  const ramped = Math.min(t, RAMP_SECONDS);
  const climbing =
    MIN_RATE * ramped +
    ((MAX_RATE - MIN_RATE) * ramped ** 3) / (3 * RAMP_SECONDS ** 2);
  return Math.floor(climbing + MAX_RATE * Math.max(0, t - RAMP_SECONDS));
}
